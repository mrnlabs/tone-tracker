...
function onFileChange(event) {
  ...
  if (props.fileType === 'pdf' && !selectedFile.type.includes("pdf")) {
    file.value = null;
  ...
  //if its not image 
  if (props.fileType === 'image' && !selectedFile.type.includes("image")) {
    file.value = null;
    fileError.value = "Only image files are allowed";
    return;
  }

  //if file is larger than 250MB
...
function onDrop(event) {  
  ...
  if (props.fileType === 'pdf' && !droppedFile.type.includes("pdf")) {
    file.value = null;
    fileError.value = "Only pdf files are allowed";
    return;
  }
...
<template>
  ...
  >
    <div class="text-center">
      ...
      <p class="mt-2 text-white">Drag and drop your file here or <label :for="docId" class="text-primary" style="cursor: pointer;">select file to upload</label></p>
      <input :id="docId" type="file" :accept="accept" class="d-none" @change="onFileChange">
    </div>
  </div>
